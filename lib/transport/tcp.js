...
(function() {
  ...
  this.alloc_p = function() {
    ...
    try {
      ...
      server.listen(0, process.env.OPENSHIFT_NODEJS_IP, function() {
...
(function() {
  ...
  this.createWebSocketClient = function(path, headers) {
    // for OPENSHIFT
    // return new websocket.Client('ws://127.6.250.1:' + this.$port + path, undefined,
    // var wsUrl = 'ws://'+process.env.OPENSHIFT_NODEJS_IP+'/';
    return new websocket.Client('ws://'+process.env.OPENSHIFT_NODEJS_IP+':' + this.$port + path, undefined,
      {
        headers: _.extend(
