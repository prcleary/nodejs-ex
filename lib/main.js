...
var configFilePath = process.env.OPENSHIFT_REPO_DIR + 'shiny-server.conf';
...
app.use(connect_util.filterByRegex(
  ...
  function (req, res, next) {
    ...
    if (req.url === 'sockjs-0.3.min.js' || req.url === 'shiny-server.js' || req.url === 'shiny-server.css')
      assets(req, res);
    else
...
var loadConfig_p = qutil.serialized(function() {
  ...
  .then(function(configRouter) {
    ...
    sockjsServer = proxy_sockjs.createServer(metarouter, schedulerRegistry, 
      configRouter.getWebsocketsDisabled());
